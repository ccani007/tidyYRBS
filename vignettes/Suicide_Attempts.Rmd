---
title: "Suicide Attempts in Adolescents: Risky Behaviors, Traumatic Events, and Hopelessness as Risk Factors"
...
V_interest <- c("record", "state.x", "district.x", "year.x", "weight.x",
                ...
                "q16", "q17","q22", "suicide_considered","suicide_attempts", "is_hopeless",
...
analysis_attempts_df <- 
 ...
 rename(
  ...
  mutate(time = Year - 2003, 
         year2 = time*time, 
         year3 = time*time*time) # Incluir la tendencia del tiempo 


table(analysis_attempts_df$time)
```


# Background
 Suicidal attempts among youth are a major public health concern in the United States of America. Nationwide, between 2009 and 2019, significant increases in the prevalence of adolescents who attempted suicide were evidenced (Jones et al., 2022). Consideration of specific individual characteristics such as risky behaviors, traumatic events, and feelings of hopelessness are not widely evaluated in research, although they may trigger suicide attempts (Bae et al., 2005). Therefore, this study aims to replicate a model proposed by Bae and colleagues (2005) that considered individual factors, behavioral factors, and exposure to traumatic events to test if they remain as predictors of suicide attempts in U.S high school students. 

# Method
The data used to conduct the analysis comes from the Youth Risk Behavioral Surveillance System (YRBS) combined data set for High School students. The study was conducted using logistic regression with data from the surveys collected between 2003 and 2019 including 412,754 adolescents, considering that the previous model was fitted into data from 2001.  The analysis considered the sample weights, so over 11 million observations were included
...
proportions_df <-
  ...
  summarise(
    ...
    propConsidered = survey_mean(suicide_attempts, na.rm = TRUE),
...
# The model
model_1 <- (svyglm(suicide_attempts ~ Sex  + Grade + Race + injured_weapon + physical_fight + hurt_partner + is_hopeless + forced_sexual + consumed_alcohol + consumed_chemical + offered_drugs + used_hallucinogenic + describe_weight, 
                  family = binomial, 
                  design = analysis_weighted_attempts_ls, 
                  na.action = na.omit))

model_y <- (svyglm(suicide_attempts ~ Sex + time + year3 + Grade + Race + injured_weapon + physical_fight + hurt_partner + is_hopeless + forced_sexual + consumed_alcohol + consumed_chemical + offered_drugs + used_hallucinogenic + describe_weight, 
                  family = binomial, 
                  design = analysis_weighted_attempts_ls, 
                  na.action = na.omit))
summary(model_y)
confint(model_y)
exp(cbind(OR = coef(model_y), confint(model_y)))


```

Results show that being injured by a weapon (OR = 1.36, 95% CI  = [1.08-1.71], p < 0.01), being involved in a physical fight (OR = 1.33, 95% CI  = [1.11 - 1.59], p < 0.01) , being forced to have sexual relationships  (OR = 2.09 , 95% CI [1.59 - 2.52],  p <0.01), consuming alcohol (OR = 1.34, 95% CI [1.11 - 1.61], p <0.01), inhaling a chemical to get high  (OR = 1.31, 95% CI  [1.00-1.71], p < 0.01), consuming hallucinogenic drugs  (OR = 1.41, ci [1.02-1.93], p < 0.05), feeling hopeless  (OR = 1.77 , 95% C1 [1.51 – 2.07], p < 0.01), being forced to have sexual relationships  (OR = 2.00 , 95% CI [1.59 - 2.52],  p <0.01), being underweight (OR = 1.61 , 95% CI [1.52 - 2.62],  p <0.01), and overweight (OR = 1.55 , 95% CI [1.33 – 2.14],  p <0.01), are significant predictors of attempting suicide when controlling for sex and grade.
Additionally, patterns of risk behaviors are different between sexes; in males physical fight, consuming chemicals to get high, being underweight and using hallucinogenic drugs are not significant predictors, while for females they are. Additionally, for females, being sexually abused showed higher odd ratios (OR = 2.17 , 95% CI [1.67 – 2.83],  p <0.01),  compared to males (OR = 1.67 , 95% CI [1.08 – 2.58],  p <0.05). 
...
model_girls <- (svyglm(suicide_attempts ~ Year + Grade + Race + injured_weapon + 
                        physical_fight + hurt_partner + is_hopeless + forced_sexual + 
                        consumed_alcohol + consumed_chemical + offered_drugs + 
                        used_hallucinogenic + describe_weight, 
                      family = binomial, 
...
The results from this study resemble the model proposed by in Bae and colleagues (2005); the predictors included significantly increased the chances of having suicidal ideation, except for being hurt by a partner and being offered drugs in schools. Additionally, different patterns of risk factors were found between sexes. These findings impact the development of preventive programs for adolescent suicidality and promote the inclusion of suicide education curricula in high school health programs. Providing evidence of risk factors that have not been widely considered before and that are found to be consistently important in the prediction of suicide attempts over the years will raise awareness and promote strategies for identifying at-risk adolescents. 