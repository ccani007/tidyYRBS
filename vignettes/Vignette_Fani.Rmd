...
proportions_every_predictor_df <-
  ...
  summarise(
    ...
    #porpHallucinogenic = survey_mean(used_hallucinogenic, na.rm = TRUE),
     propSubstance = survey_mean(consumed_alcohol, na.rm = TRUE) 
    + survey_mean(used_hallucinogenic, na.rm = TRUE) 
    + survey_mean(consumed_chemical, na.rm = TRUE),
    .groups = "keep"
  )%>% 
  mutate(Year = as.character(Year)) %>% 
  na.omit()
